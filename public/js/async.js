...
var page_number = 1;
/* обрабатываем нажатие на кнопку ShowMore в каталоге */
function click_on_show_more() {
    page_number++;
    $.ajax({
        type: 'POST',
        url: '../controller/catalog.php',
        data: 'action=show_more'+'&page_number='+page_number,
        success: function(data){
                $('#show_more').remove();
                $('div.catalog_list').append(data);
        }
    });
}